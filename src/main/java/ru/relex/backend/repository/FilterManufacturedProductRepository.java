...
public interface FilterManufacturedProductRepository {
     ...
     * <p>
     * При осуществлении выборки происходит группировка по полю {@link ProductDetailFilter#productId},
     * а в качестве агрегирующего значения используется количество произведенных единиц товара.
     ...
     * <p>
     * Для динамического составления предиката используется {@link QPredicates}. Составление предиката динамически
     * становится возможным благодаря использованию библиотеки QueryDSL. Данный подход по сравнению со встроенным
     * решением с помощью Criteria Api помогает значительно увеличить читаемость запроса, поскольку сам запрос
     * составлен на основе {@link JPAQuery}, который имеет SQL-подобный синтаксис.
     *
